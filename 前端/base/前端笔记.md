...
```css
.monaco-workbench .part>.content{font-size:13px;font-family:"JetBrains Mono ExtraBold";font-weight:bold;}
```
...
```shell
git push -o merge_request.create -o merge_request.target={my-target-branch} -o merge_request.title="MR标题" -o merge_request.description="MR描述"

# 推送代码到 gitee远程仓(假如你有gitee/github两个远程仓)
# merge_request.create 创建MR
# merge_request.target 目标分支(以release1.0为例)
# merge_request.title MR标题
# merge_request.description MR描述
git push gitee -o merge_request.create -o merge_request.target=release1.0 -o merge_request.title="MR标题" -o merge_request.description="MR描述"


git push gitee -o merge_request.create -o merge_request.target=release1.0 -o merge_request.title="厂区固定写死" -o merge_request.description="厂区固定写死"
```
